...
import { users, utils } from '../../../mock/index';
...
describe('[Session] store/reducer', () => {
    ...
    it('should reduce session restore states', () => {
        expect(sessionReducer(sessionInitialState,
            new fromSessionActions.SessionRestoreInitAction(session)))
            .toEqual(merge(cloneDeep(state), { session: session, error: null, state: EntityState.Progress }));
        expect(sessionReducer(sessionInitialState,
        ...
        expect(sessionReducer(sessionInitialState,
            new fromSessionActions.SessionRestoreFailureAction('error')))
